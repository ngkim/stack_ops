...
def pool_trace(func):    
    ...
    def callfunction(*args, **kwargs):
        ...
        # print("# Function[%s(%s)(%s)]" % (func.__name__, args, kwargs))
        start = time.time()
        r = func(*args, **kwargs)
        end = time.time()
        elapsed = end - start
        print("  # Function[%s] elapse [%s] returned [%s]\n" % (func.__name__, elapsed, r))
        return r
...
class WorkerThread(threading.Thread):
    ...
    """    
    # @pool_trace # -> make error !! i don't know why
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                print "   inside worker thread :: request %s" % request
            except Queue.Empty:
                continue
            else:
                if self._dismissed.isSet():
...
class ThreadPool:
    ...
    def createWorkers(self, num_workers, poll_timeout=5):
        ...
        for i in range(num_workers):
            wt = WorkerThread(self._requests_queue, self._results_queue, poll_timeout=poll_timeout)
    ...
    def putRequest(self, request, block=True, timeout=None):
        ...
        print "    inside pool :: workRequests size %d" % len(self.workRequests)
...
if __name__ == '__main__':
    ...
    def task_callback(request, result):
    ...
    requests = makeRequests(do_task, data, task_callback, handle_exception)
    # to use the default exception handler, uncomment next line and comment out
    # the preceding one.
    #requests = makeRequests(do_task, data, task_callback)

    # or the other form of args_lists accepted by makeRequests: ((,), {})
    data = [((random.randint(1,10),), {}) for i in range(20)]
    requests.extend(
        makeRequests(do_task, data, task_callback, handle_exception)
        #makeRequests(do_something, data, task_callback)
    ...
    for req in requests:
        ...
        main.putRequest(req)        
        print "%d] Work request #%s added." % (cnt, req.requestID)
    # or shorter:
